...
elist = ['天生丽质','慧质兰心','秀外慧中','暗香盈袖','倾国倾城','温婉娴淑','千娇百媚','仪态万千','美艳绝世','国色天香','花容月貌','明目皓齿','淡扫峨眉','清艳脱俗','香肌玉肤','清丽绝俗','仪态万端','婉风流转','美撼凡尘','聘婷秀雅','娥娜翩跹','俏丽多姿','如花似月']
xf =['闭月羞花','沉鱼落雁','倾国倾城','千娇百媚','明目皓齿','淡扫峨眉','清艳脱俗','香肌玉肤','婉风流转','美撼凡尘','聘婷秀雅','娥娜翩跹','俏丽多姿','风姿卓越','顾盼流转','清丝纠缠','举步轻摇','剪水双瞳','美艳绝伦','神仙玉骨','如花似玉','一代佳人','一代容华','红粉佳人','红粉青娥','天生尤物','天仙化人','姑射神人','绝色美人','月里嫦娥','人面桃花','天生丽质','天香国色','仙资玉质','妍资艳质','夭桃秾李','色艳桃李','华如桃李','桃羞杏让','如花似月','羞花闭月','花容月貌','芙蓉如面','粉白黛黑','娇艳惊人','冠觉群芳','桃腮杏脸','樱桃小口','婀娜多姿','丰姿绰约','娉婷袅娜','国色天姿','倾国倾城','国色天香','婀娜多姿','亭亭玉立','粉面桃花','肤如凝脂','声若莺啼','明眸皓齿','花容月貌','风华绝代','楚楚动人','冰清玉洁','杨柳细腰','气质清纯','容颜秀美','婉约可人','天生丽质','美丽大方','清秀大方','体态轻盈','气质高雅','温柔体贴','知书达礼','仪态万千','艳冠群芳','楚楚动人','秀外慧中','风情万种','知书达理','温文尔雅','空谷幽兰','超凡脱俗','兰心蕙质','秀外慧中']
xf.extend(elist)
delist = ['善良','温柔','善解人意','小鸟依人','聪明','可爱','文静','活泼','冰雪聪明','天使般','','美丽','可人','羞涩','艳丽','妍丽','美艳','富丽','瑰丽','秀丽','鲜艳','绚丽','漂亮','摩登','大方','时兴','入时','时髦','锦绣','姣好','俊美','俊丽','俊秀','俊俏','斑斓','大度','文雅','标致','美好','优美','艳丽','完美','标致','美妙','美观','俊俏 ','气质','端庄','明媚皓齿']
delist.extend(xf)
# print(delist)
greetList = ['春水初生,春林初盛,春风十里,不如睡你。',
 '外面风大，跟我回家。',
 '留在我身边，远近我都接受。',
 '向来没耐心的我，在你身边徘徊了这么久。',
 '你站的方向风吹过来都是暖的。',
...
def get_weather():
    while True:
        try:
            url = "https://tianqi.moji.com/today/china/zhejiang/beilun-district"
            r = requests.get(url)
            content = r.text
            ...
            # 通过类名查找
            now = soup.select('.now')
            contents = soup.select('.detail_ware_title')
            day = soup.select('.day')
            days = []
            for i in day:
                xe = i.get_text().split()
...
def get_news1():
    while True:
        try:
            ...
            print('词霸抓取错误时间'%(time.asctime( time.localtime(time.time()) )))
            time.sleep(1)
            continue
    
        
def moring_news(sendfriend="hid"):
   
        try:
            my_friend = bot.friends().search('安生')[0] # 你朋友的微信昵称，不是备注，也不是微信帐号。"安生。"
            print(my_friend.send('%s.早安！' %get_news1()))        # 此处获取的是get_news1()方法返回列表的第一部分内容
            print(my_friend.send('给你的一百封信之--%s' % (random.sample(lelist, 1)[0]))       )
            return 123
        except:
            my_friend = bot.friends().search("安生")[0]   # 你的微信昵称，不是微信帐号。
            print(my_friend.send('%s早安！' %get_news1()))        # 此处获取的是get_news1()方法返回列表的第一部分内容
            print(my_friend.send('给你的一百封信之--%s' % (random.sample(lelist, 1)[0]))       )
            print(u"今天早上5.20的消息发送失败了")
            print(time.asctime( time.localtime(time.time()) ))
            
def weather(sendfriend="hid"):
  
        try:
            my_friend = bot.friends().search('安生')[0] # 你朋友的微信昵称，不是备注，也不是微信帐号。"安生。"
            clothes = get_weather()[-1][1].replace('墨迹天气建议您','师兄建议%s的小仙女罗应该' %(random.sample(delist, 1)[0]))
            all_day = get_weather()[1]
            day_morning = get_weather()[0][0]
            day_night = get_weather()[0][1]
            print(my_friend.send('今天实况天气为%s,温度为%s摄氏度,湿度为%s,风向为%s%s。白天天气是%s,最高温度为%s摄氏度,日出时间是%s。夜晚天气是%s,最低温度为%s摄氏度,日落时间是%s。今天%s' %(all_day[2],all_day[1],all_day[4],all_day[-2],all_day[-1],day_morning[1],day_morning[2],day_morning[-1],day_night[1],day_night[2],day_night[-1],clothes)))
            # print(my_friend.send('%s' %mes))        # 此处获取的是get_news1()方法返回列表的第一部分内容
            return 12
        except:
            my_friend = bot.friends().search("hid")[0]   # 你的微信昵称，不是微信帐号。
            my_friend.send(u"今天的天气消息发送失败了")
            print(time.asctime( time.localtime(time.time()) ))
          

def night_news(sendfriend="hid",Y=2013,M=9,D=29):
        ...
        try:
            ...
            my_friend = bot.friends().search("hid")[0]   # 你的微信昵称，不是微信帐号。
            my_friend.send(u"今天晚上的消息发送失败了")
            print(time.asctime( time.localtime(time.time()) ))
           

schedule.every().day.at("5:20").do(moring_news)
schedule.every().day.at("7:00").do(weather)
schedule.every().day.at("21:20").do(night_news)


while True:
    schedule.run_pending()
    print(time.asctime( time.localtime(time.time()) ),random.sample(delist, 2)[0])
